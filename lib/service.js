...
class Service extends AdapterBase {
  ...
  $findAggregate(params = {}) {
    ...
    const modelSchemaExtendedByJoinedReferences = model.schema.clone();
      ...
      if (pipe.$lookup.from && pipe.$lookup.localField) {
        ...
        if (joinedModel) {
          Object.keys(joinedModel.schema.paths).forEach(key => {
            modelSchemaExtendedByJoinedReferences.paths[`${pipe.$lookup.localField}.${key}`] = joinedModel.schema.paths[key];
          })
        }
      }
    });
    ...
    if (query) {
      ...
      if (matchPipeIndex !== -1) {
        ...
        aggregationPipeline[matchPipeIndex] = {
          $match: cast(modelSchemaExtendedByJoinedReferences, {
        ...
        aggregationPipeline.push({
          $match: cast(modelSchemaExtendedByJoinedReferences, { ...query })
        });
      }
    }
